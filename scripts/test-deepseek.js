  ...
  try {
    ...
    if (fs.existsSync(envPath)) {
      ...
      for (const line of lines) {
        ...
        if (match) {
          // Remove potential quotes or carriage returns
...
async function testDeepSeek() {
  ...
  if (!apiKey) {
    ...
    console.log('DEEPSEEK_API_KEY=tu_api_key_de_deepseek\n');
    process.exit(1);
  }

  console.log('🔑 API Key detectada. Iniciando prueba con DeepSeek...');
  ...
  try {
    ...
    console.log('✅ Respuesta recibida exitosamente de DeepSeek!\n');
    console.log('--- JSON Adaptado ---');
    console.log(JSON.stringify(JSON.parse(content), null, 2));
    console.log('---------------------');
  } catch (err) {
    console.error('\n❌ Falló la conexión con la API de DeepSeek:');
