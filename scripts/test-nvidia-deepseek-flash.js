...
async function testNvidiaDeepSeekFlash() {
  ...
  console.log(`🔑 Clave detectada. Conectando con DeepSeek Flash en NVIDIA NIM (${modelName})...`);
  ...
  try {
    ...
    // Clean up potential reasoning thinking block (<think>...</think>) if it outputs it
    if (content.includes('</think>')) {
      content = content.split('</think>')[1].trim();
    }

    console.log('✅ Respuesta recibida exitosamente de DeepSeek Flash en NVIDIA!\n');
    console.log('--- JSON Adaptado ---');
    console.log(JSON.stringify(JSON.parse(content), null, 2));
    console.log('---------------------');
  } catch (err) {
    console.error('\n❌ Falló la conexión con DeepSeek Flash en NVIDIA:');
    console.error(err.message);
  }
}

testNvidiaDeepSeekFlash();
