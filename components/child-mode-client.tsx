...
function AccessFromTaskOnly() {
  return (
    <main className="min-h-screen bg-amiko-navy px-5 py-8 text-white">
      <section className="mx-auto flex min-h-[calc(100vh-64px)] max-w-[430px] flex-col items-center justify-center text-center">
        <div className="flex h-20 w-20 items-center justify-center rounded-full bg-white/10">
          <AmikoIcon name="task" className="h-10 w-10 text-amiko-green" />
        </div>
        <p className="mt-6 text-xs font-black uppercase tracking-[0.18em] text-amiko-green">
          Modo estudiante
        </p>
        <h1 className="mt-3 text-3xl font-black leading-tight">
          Abre este modo desde una tarea
        </h1>
        <p className="mt-4 max-w-sm text-base font-bold leading-7 text-blue-100">
          Primero un adulto adapta o asigna la tarea. Después AMIKO la muestra paso a paso para el estudiante.
        </p>
        <Link
          href="/dashboard"
          className="focus-ring mt-8 inline-flex min-h-12 items-center justify-center rounded-full bg-amiko-green px-6 text-sm font-black text-white shadow-card"
        >
...
export function ChildModeClient() {
  ...
  const validTaskOrigin = searchParams.get("from") === "task" || searchParams.get("from") === "adapted-task";
  ...
  // Refactor futuro: la ruta/carpeta `child-mode` deberia migrar a `student-portal`
  // cuando se cambien rutas y enlaces de forma coordinada.
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-amiko-navy text-white">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,#0F5AD1_0%,transparent_34%),linear-gradient(180deg,#09367C_0%,#071D41_100%)] opacity-95" />

      <header className="sticky top-0 z-30 border-b border-white/10 bg-amiko-navy/95 backdrop-blur-xl">
        <div className="mx-auto grid max-w-[430px] grid-cols-[64px_1fr_64px] items-center gap-2 px-4 py-3">
          <button
            type="button"
            onClick={() => router.push("/dashboard")}
            className="focus-ring flex h-11 w-11 items-center justify-center rounded-full bg-white/10 text-white"
          ...
          <div className="text-center">
            ...
            <h1 className="text-lg font-black leading-tight">{adaptedTask.subject}</h1>
          </div>
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-64px)] max-w-[430px] flex-col px-4 py-5">
        <section className="rounded-[28px] border border-white/10 bg-white/10 p-4 shadow-soft backdrop-blur">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="rounded-2xl bg-white px-4 py-2 text-right text-amiko-navy">
              <p className="text-[10px] font-black uppercase tracking-[0.12em] text-amiko-muted">
                Queda
              </p>
        ...
        {sessionMessage ? (
          <section className={`mt-4 rounded-[24px] border p-4 shadow-card ${messageClasses(sessionMessage.tone)}`}>
            ...
            {mustStop ? (
              <p className="mt-3 rounded-2xl bg-white/70 px-3 py-2 text-xs font-black text-amiko-navy">
        ...
        <section className="flex flex-1 items-center py-5">
          {successMessage ? (
            <div className="w-full rounded-[32px] bg-white p-7 text-center text-amiko-ink shadow-soft">
              ...
              <Link
                href="/dashboard"
                className="focus-ring mt-6 inline-flex min-h-12 items-center justify-center rounded-full bg-amiko-green px-6 text-sm font-black text-white"
              >
                Volver al inicio adulto
              </Link>
            </div>
            ...
            <div className="w-full rounded-[32px] bg-white p-6 text-center text-amiko-ink shadow-soft">
              ...
              <div className="mx-auto mt-6 max-w-xs rounded-[24px] border border-amiko-blue/10 bg-amiko-sky p-4">
                <p className="text-xs font-black uppercase tracking-[0.14em] text-amiko-blue">
                  Puedes mirar esto
                </p>
                <p className="mt-2 text-xl font-black text-amiko-blue">{currentStep.visualSupport}</p>
              </div>

              <div className="mx-auto mt-5 max-w-sm rounded-[22px] bg-amiko-cream px-4 py-3 text-sm font-bold leading-6 text-amiko-navy">
                {lastAction}
              </div>
        ...
        {!successMessage ? (
          <section className="grid gap-3 pb-4">
            {mustStop ? (
              <div className="rounded-[24px] border border-white/15 bg-white/10 p-4 text-center text-sm font-bold leading-6 text-blue-100">
              ...
              <>
                <button
                  ...
                  className="focus-ring min-h-16 rounded-[24px] bg-amiko-green px-5 text-xl font-black text-white shadow-card active:scale-[0.98]"
                >
                  Lo hice
                </button>
                <button
                  type="button"
                  onClick={askHelp}
                  className="focus-ring min-h-16 rounded-[24px] bg-amiko-blue px-5 text-xl font-black text-white shadow-card active:scale-[0.98]"
                >
                  Necesito ayuda
                </button>
                <button
                  ...
                  className="focus-ring min-h-16 rounded-[24px] border border-white/15 bg-white px-5 text-xl font-black text-amiko-navy shadow-card active:scale-[0.98]"
                >
                  Necesito pausa
                </button>
              </>
      ...
      {pauseOpen && !mustStop ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-sm rounded-[32px] bg-white p-6 text-center text-amiko-ink shadow-soft">
            ...
            <button
              ...
              className="focus-ring mt-6 min-h-12 w-full rounded-full bg-amiko-green px-6 text-sm font-black text-white"
            >
              Entendido
            </button>
          </div>
